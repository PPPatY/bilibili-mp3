...
- 每个视频间隔5秒；如遇风控或报错可临时退回到更长的间隔（需修改 `extension/batch.js` 中的 `interval`）
...
批量下载采用串行模式（一次一个视频），每个视频间隔5秒。这是为了在速度和B站限流之间取得平衡。一个50集的合集大约需要25-35分钟（取决于每集时长）。
